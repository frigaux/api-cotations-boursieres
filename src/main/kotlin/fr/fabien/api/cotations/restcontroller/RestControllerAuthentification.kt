...
@Tag(name = "API pour l'authentification")
@RestController
@RequestMapping("bourse/authentification")
class RestControllerAuthentification(
    private val serviceJWT: ServiceJWT
) {
    @Operation(summary = "Récupération du JsonWebToken")
    @ApiResponses(
        value = [
            ApiResponse(
                responseCode = "200", description = "Le JWT",
                content = [Content(
    ...
    private fun generateJwtToken(@Valid @RequestBody dtoAuthentification: DtoAuthentification): DtoJWTWrapper {
